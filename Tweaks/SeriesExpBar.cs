...
public unsafe class SeriesExpBar : Tweak
{
    ...
    public override string Description => "In PvP areas the experience bar shows series rank and experience instead.";
    ...
    // TODO: SimpleTweaks hooks this too, which makes text not appear as it should.. trying to find a solution
    ...
    private IntPtr OnRequestedUpdateDetour(AddonExp* addon, NumberArrayData** numberArrayData, StringArrayData** stringArrayData)
    {
        ...
        if (Service.ClientState.LocalPlayer?.ClassJob.GameData == null || !Service.Data.IsDataReady || !Service.ClientState.IsPvP)
        ...
        // max value is set to 10000 in AddonExp_OnSetup, so no touchy
