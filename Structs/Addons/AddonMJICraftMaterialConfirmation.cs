...
// ctor "40 53 48 83 EC 20 48 8B D9 E8 ?? ?? ?? ?? 48 8D 05 ?? ?? ?? ?? 41 B0 01 48 89 03 BA ?? ?? ?? ?? 33 C0 48 8B CB 48 89 83 ?? ?? ?? ?? 48 89 83 ?? ?? ?? ?? 48 89 83 ?? ?? ?? ?? 48 89 83 ?? ?? ?? ?? 48 89 83 ?? ?? ?? ?? B8"
[VTableAddress("48 8D 05 ?? ?? ?? ?? 41 B0 01 48 89 03 BA ?? ?? ?? ?? 33 C0 48 8B CB 48 89 83 ?? ?? ?? ?? 48 89 83 ?? ?? ?? ?? 48 89 83 ?? ?? ?? ?? 48 89 83 ?? ?? ?? ?? 48 89 83 ?? ?? ?? ?? B8 ?? ?? ?? ??", 3)]
...
public unsafe partial struct AddonMJICraftMaterialConfirmation
{
    ...
    [VirtualFunction(47)]
    public partial void OnSetup(AddonMJICraftMaterialConfirmation* addon, int numAtkValues, AtkValue* atkValues);
}
