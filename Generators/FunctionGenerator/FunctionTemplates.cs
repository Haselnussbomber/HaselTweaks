namespace HaselTweaks.Generators.FunctionGenerator
{
    ...
    public unsafe static partial class Resolver {
        private static void InitializeMemberFunctions(SigScanner s)
        {
            ...
            try {
                ...
                PluginLog.Warning($""function {{ struct.name }}::{{ mf.name }} failed to match signature {{ mf.signature }} and is unavailable"");
            }
            {{~ end ~}}
            {{~ end ~}}
        }
