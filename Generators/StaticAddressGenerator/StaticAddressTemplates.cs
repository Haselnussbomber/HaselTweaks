namespace HaselTweaks.Generators.StaticAddressGenerator
{
    ...
    public unsafe static partial class Resolver {
        private static void InitializeStaticAddresses(SigScanner s)
        {
            ...
            try {
                ...
                PluginLog.Warning($""static address {{ struct.name }}::{{ sa.name }} failed to match signature {{ sa.signature }} and is unavailable"");
            }
            {{~ end ~}}
            {{~ end ~}}
        }
